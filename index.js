...
function customObjectFromEntries(entries) {
    ...
    const fromEntriesArr = Object.fromEntries(newArr);
...
function checkProperty(obj, prop) {
    ...
    const ownPropObj = obj.hasOwnProperty(prop);
    return ownPropObj;
...
function keysAndValues(obj) {
    ...
    const newArr = Array.of(keyObj, valueObj);
    return newArr;
...
function filterObjectsByKey(arr, key) {
    ...
    const filterArr = arr.filter((elem) => elem[key]);
    return filterArr;
...
function checkOwnership(obj1, obj2, key) {
    const objOneHas = obj1.hasOwnProperty(key);
    const objTwoHas = obj2.hasOwnProperty(key);
    if (!objOneHas && !objTwoHas) {
        return false;
    }
    return Object.is(objOneHas, objTwoHas);
...
function getObjectValuesSum(obj) {
    ...
    const valueObj = Object.values(obj);
    return valueObj.reduce((a, b) => a + b, 0);
